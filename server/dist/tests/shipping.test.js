...
describe("Shipping Calculator - Tests Fonctionnels", () => {
    ...
    test.each(distanceCases)("%s km / %s kg / %s -> %s (€) : %s", (distance, weight, type, expected) => {
        expect((0, shipping_1.calculateShipping)(distance, weight, type)).toBe(expected);
    });
});
it("devrait lever une erreur si la distance est négative", () => {
    expect(() => (0, shipping_1.calculateShipping)(-10, 5, "standard"))
        .toThrow("Distance invalide");
});
it("devrait lever une erreur si le poids est <= 0", () => {
    expect(() => (0, shipping_1.calculateShipping)(10, 0, "standard"))
        .toThrow("Poids invalide");
});
it("devrait lever une erreur si le poids est > 50", () => {
    expect(() => (0, shipping_1.calculateShipping)(10, 60, "standard"))
        .toThrow("Poids invalide");
});
